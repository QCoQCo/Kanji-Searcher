...
const RelatedWords: React.FC<RelatedWordsProps> = ({
    ...
    const fetchRelatedWords = async () => {
        ...
        try {
            ...
            // 각 한자로 검색하여 관련 단어 수집
            for (const kanji of currentKanji.slice(0, 3)) {
                // 최대 3개 한자만 검색
                try {
            ...
            const uniqueWords = Array.from(relatedWords)
                ...
                .filter((word) => {
                    ...
                    if (currentJLPTLevels.length > 0) {
                        return (
                            word.jlpt &&
                            Array.isArray(word.jlpt) &&
                            word.jlpt.some((level: string) => currentJLPTLevels.includes(level))
                        );
                    }
                    return true;
                })
                                ...
                                {recommendations.map((word, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className='rec-meaning'>
                                            ...
                                            {word.senses
                                                ?.flatMap(
                                                    (sense: any) => sense.english_definitions || []
                                                )
                                                .filter(Boolean)
                                                .slice(0, 2)
                                                .join(', ') || '의미 없음'}
                        ...
                        <div className='jlpt-recommendations'>
                            ...
                            <div className='jlpt-stats'>
                                {currentJLPTLevels.map((level) => (
                                    <span
                                        key={level}
                                        className={`jlpt-badge jlpt-${level.toLowerCase()}`}
                                    >
                                        {level}
                                    </span>
                                ))}
